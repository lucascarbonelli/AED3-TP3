...
vector<pair<unsigned int,float> > tournament(matchBoard board, vector<individual>& population, int iter){
  ...
  for (int i = 0; i < population.size(); ++i){
    for (int j = i+1; j < population.size(); ++j){
      ...
      scores[i].second += matchRes.first.won/matchRes.first.median_l_time - matchRes.first.lost/matchRes.first.median_w_time;
      scores[j].second += matchRes.second.won/matchRes.second.median_l_time - matchRes.second.lost/matchRes.second.median_w_time;
    }
  }
  return scores;
}
...
vector<individual> genetic_optimization(matchBoard board, unsigned int pop_size, unsigned int ind_size, unsigned int max_generations, unsigned int max, unsigned int iter){
...
int main(){
  ...
  board.w1_first = true;
  int iter = 3;
  vector<individual> population = genetic_optimization(board,10,15,20,100,iter);
