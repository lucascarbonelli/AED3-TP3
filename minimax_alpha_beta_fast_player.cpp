...
// deep = profundidad del arbol, catidad de niveles que faltan recorrer, cuando llega a 0, para
int minimaxAlphaBetaFast(Board &b, bool miniMax, int alpha, int beta, int deep){
    int result;

    if(b.didIWin()) {result = 1;}
    else if(b.didILost()) {result = -1;}
    else if(b.getPieces() == 0 || b.isBoardFull() || deep == 0) {result = 0;} // Empate
    else{
        if(miniMax){
            // Buscamos maximizar
            result = -1;
    ...
    while (true) {
        ...
        if (go_first == "vos"){
            ...
            for(int i = 0; i < board.getColumns(); i++){
                ...
                nextLevelBoard.addPlayer(i);
                int maxInThisBranch = minimaxAlphaBetaFast(nextLevelBoard, false, -1, 1, levelsToExplore);
