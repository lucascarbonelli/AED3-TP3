...
void exploreNeighbourhood(int drop_set,vector<int>& init_weights, int M, int N, int c , int P, vector<int> range){

  switch (drop_set) {
    case 1:
     for (int i = 0; i < c-2; i++) init_weights[i] = 0;
     break;
    case 2:
      init_weights[c-2] = 0;
      break;
    case 3:
      for (int i =  c-1; i <  c-1 + M ; i++) init_weights[i] = 0;
      break;
    case 4:
      for (int i =  c-1 + M ; i < c-1 + M + (c-2); i++) init_weights[i] = 0;
      break;
    case 5:
      for (int i = c-1 + M + (c-2) ; i < c-1 + M + (c-2) +(c-2); i++) init_weights[i] = 0;
      break;
  }
  ...
  vector<int> weights = drop_set == 1 ? init_weights : gridSearchHelper(params_to_optimize,range,init_weights,N,M,c,P);


  /* optimizamos parametros del segundo feature */
  params_to_optimize.clear();
  params_to_optimize.push_back(c-2);
  weights = drop_set == 2 ? weights : gridSearchHelper(params_to_optimize,range,weights,N,M,c,P);


  /* optimizamos parametros del cuarto feature */
  params_to_optimize.clear();
  for (int i = c-1 + M ; i < c-1 + M + (c-2) ; i++) params_to_optimize.push_back(i);
  weights = drop_set == 4 ? weights : gridSearchHelper(params_to_optimize,range,weights,N,M,c,P);
  ...
  weights = drop_set == 5 ? weights : gridSearchHelper(params_to_optimize,range,weights,N,M,c,P);

}



vector<int> GRASP(int drop_set,int n, int m, int c, int p, vector<int> range, int iterations){
  ...
  for (size_t i = 0; i < iterations; i++) {
    ...
    exploreNeighbourhood(drop_set,weights,m,n,c,p,range);
...
int main(int argc, char const *argv[]) {
  ...
  for(int i = 1; i < 6;i++){
    vector<int> weights = GRASP(i,rows,columns,c,p,range,10);
